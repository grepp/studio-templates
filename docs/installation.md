...
| 환경 | 선택할 방식 | 예시 파일 |
|------|------------|-----------|
| 서버 런타임 (Node.js, Python 등) | `.env` | `.env.example` |
| 브라우저 only (HTML/JS) | JS 설정 파일 | `app-config.example.js` |
| 빌드 도구 사용 (Vite, webpack 등) | `.env` + 빌드 주입 | `.env.example` |
...
- 코드에 하드코딩된 비밀번호 / API Key가 없는지 확인
  - 발견 시: 값을 **참조로 대체**하고 dummy 값을 예시 파일에 넣는다. **값을 삭제하지 마라.**
- 실제 설정 파일(`.env`, `app-config.js` 등)이 `.gitignore`에 포함되어 있는지 확인
...
⚠️ 앱을 실행하려면 다음 단계가 필요합니다:
1. {예시 파일}을 복사하여 실제 파일을 만드세요:
   예: cp {예시 파일} {실제 파일}
2. 실제 파일에 실제 API Key, 비밀번호 등을 입력하세요.
3. 기존에 Git에 커밋된 민감정보가 있었다면 해당 값을 즉시 재발급받으세요.
   (이미 GitHub 등에 공개된 API Key는 무효화되었을 가능성이 높습니다)
